...
parser.add_argument("-html", "--HTML", help="HTML File complete Path with extension i.e C:\<path>\example=page.html",
                    required=True)
parser.add_argument("-excel", "--EXCEL",
                    help="EXCEL File complete Path with extension i.e C:\<path>\LstMetadataprofiltest_formatted.xlsx",
                    required=True)
parser.add_argument('-config', '--CONFIG', help='delimited list input i.e 0,1,33,34,35',
...
df = df[df[len(
    df.columns) - 1] != 1]  # Remove rows based on column values i.e If Last Column (Remove_Rows) value is 1 remove complete row ignore.
...
sub_set_df_fhtml = df[
    args.CONFIG[1:]]  # Select Columns that contain data excluding first column that contains foramtting info
...
# Iterate over each row of newly created table and check which class it belongs to and then make necessary
# changes/formatting
